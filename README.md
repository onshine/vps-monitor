...
在 2 核等小内存机器上更新前端时，构建进程、alist-tvbox、数据库等会同时抢 CPU，把核占满导致 SSH 卡死。此时**不该杀进程**——数据库被杀会损坏数据，构建被杀会前功尽弃。正确做法是构建前临时给其他服务降配，让出资源，构建完自动恢复。

构建前执行：

```bash
sudo vps-build-mode enter
```

它会把默认清单里的容器（`alist-tvbox`、`xiaoya-tvbox`）临时限制到 0.3 核 / 256MiB，并把每个容器的原始配额记录到状态文件。

构建完成后执行：

```bash
sudo vps-build-mode exit
```

**精确恢复**到进入前的配额——如果你之前手动设过 CPU 或内存上限，恢复的是你设的值，而不是清零。

查看当前状态：

```bash
sudo vps-build-mode status
```
...
```bash
sudo BUILD_MODE_CONTAINERS="alist-tvbox,betterforward" BUILD_MODE_CPUS=0.2 BUILD_MODE_MEM=200m vps-build-mode enter
```

> 让路只针对可安全降配的容器化服务。数据库默认不在清单里——数据库在高负载时被限速可能拖慢事务，需要你自己权衡后手动加入。
...
`/etc/vps-monitor.env` 在升级时不会被覆盖，这保护了你填写的 Token 和自定义配置。但这带来一个副作用：**当项目出于安全考虑收紧某个默认值时，老用户的配置文件里仍是旧值。**

例如 v2.6.0 把命令豁免名单从 40 条收窄到 8 条（移除 `tar`、`rsync`、`gcc`、`make` 等通用命令，因为范围过大容易被伪装绕过）。但已安装的机器升级后，配置里仍是那 40 条，收窄的安全收益拿不到。

从 v2.9.0 起，升级会自动执行一次安全迁移，判断逻辑是：

| 配置文件中的当前值 | 处理方式 |
|---|---|
| 与某个历史版本的默认值**完全一致** | 视为「用户从未改过」，更新为当前默认值 |
| 与历史默认值**有任何差异** | 视为「用户自定义」，原样保留，不做任何修改 |
| 该项**不存在** | 补充为当前默认值 |

也就是说，只要你曾用 `allow`、`allow-container` 追加过内容，或手工编辑过这一行，迁移就会跳过它。判断标准是完整字符串比对，不做模糊匹配，避免误判。

迁移过程会在升级时打印结果，例如：

```text
命令豁免名单为历史默认值，已更新为当前收窄后的默认值。
已补充容器豁免名单默认值。
```
...
```bash
sudo vps-monitorctl migrate
```

如果你希望无条件回到当前默认值，不管之前改过什么，用：

```bash
sudo vps-monitorctl reset-allow
```

两者的区别：`migrate` 保守，只在确认没改过时才动；`reset-allow` 强制覆盖。
...
报告中会单独输出 `EARLY EVIDENCE` 章节，并标注进程状态。Telegram 告警摘要也优先使用最早抓到的身份。
...
|---|---|
| `存活` | 进程仍在运行，未被监控干预 |
| `存活（已被监控降级：降级(限速)）` | 监控已限速或冻结，进程仍在 |
| `已退出（监控处置：SIGTERM）` | 由本工具处置导致退出，动作明确记录 |
| `已退出（非监控处置，退因未知）` | 本工具未干预。可能是自行结束、崩溃、内核 OOM、`docker stop` 或人工 kill |

只对本工具自己的动作做确定标记。第四种不做进一步推断——内核 OOM 日志有写入延迟，PID 又会被复用，强行归因容易误判。要确认是否 OOM，请查 `dmesg -T | grep -i oom` 或报告中的 `KERNEL/SYSTEM WARNINGS` 章节。
...
### CPU / 磁盘爆满时，授权 SIGKILL 就能自动解决吗？

不一定，而且往往不该这么做。三种常见情况：

- **主因是受保护进程**（如 mysqld、dockerd）：即使授权 SIGKILL 也不会对它生效，这是有意的——杀数据库会损坏数据，比卡机严重得多。告警会明确标注「不会自动处置（受保护…），需人工判断」并给出限速建议。
- **主因是构建或多进程混战**：单个进程可能只占一部分 CPU，杀掉它不解决问题，根因是资源总量不足。应使用「构建让路模式」或给容器设 CPU 上限，而不是杀进程。
- **磁盘读写爆满常是内存不足的连带现象**：内存耗尽触发大量 swap 换页，表现为磁盘狂读。此时该处理的是吃内存的进程或给容器设内存上限，而不是针对磁盘。

`SIGKILL` 只在极少数「确认是失控的、可安全终止的非关键进程」时才有意义。默认禁止是刻意的安全设计。

### 前端构建时 CPU 被占满、SSH 卡死怎么办？

用构建让路模式：构建前 `sudo vps-build-mode enter` 临时给其他容器降配，构建完 `sudo vps-build-mode exit` 恢复。详见上文「构建让路模式」。
